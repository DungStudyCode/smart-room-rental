...
const extractVNPhone = (text) => {
  ...
  if (matches) {
    for (let match of matches) {
      ...
      if (cleanPhone.length === 10 && cleanPhone.startsWith('0')) return cleanPhone;
    }
  }
  return null;
};
...
async function processScraping(url, isAuto = false) {
  ...
  try {
    ...
    for (let i = 0; i < maxPosts; i++) {
      ...
      const filter = post.postUrl ? { postUrl: post.postUrl } : { description: post.text.substring(0, 50) };
      ...
      const savedDoc = await Room.findOneAndUpdate(filter, newRoomData, { upsert: true, new: true });
...
const runCrawlerBot = async (req, res) => {
  if (!req.body.url) return res.status(400).json({ success: false, error: 'Thiếu URL.' });
  try {
    const savedRooms = await processScraping(req.body.url, false);
    if (savedRooms.length === 0) return res.status(400).json({ success: false, message: 'Cào thất bại hoặc bị lọc hết (Thiếu SĐT/Trùng lặp).' });
    res.status(200).json({ success: true, message: `Thành công! Đã chắt lọc được ${savedRooms.length} tin.`, data: savedRooms[0] });
  } catch (error) {
...
const getPendingRooms = async (req, res) => {
  try { res.status(200).json({ success: true, data: await Room.find({ status: 'PENDING' }).sort({ createdAt: -1 }) }); } 
...
const approveRoom = async (req, res) => {
  try {
    ...
    if (!room) return res.status(404).json({ success: false, error: 'Không tìm thấy tin.' });
    res.status(200).json({ success: true, message: 'Duyệt thành công', data: room });
  } catch (error) { res.status(500).json({ success: false, error: error.message }); }
};
...
const deleteRoom = async (req, res) => {
  try {
    ...
    if (!room) return res.status(404).json({ success: false, error: 'Không tìm thấy tin để xóa.' });
    res.status(200).json({ success: true, message: 'Đã xóa tin rác thành công!' });
  } catch (error) {
    res.status(500).json({ success: false, error: error.message });
  }
};
...
const toggleAutoBot = (req, res) => {
  ...
  if (action === 'start') {
    if (isBotRunning) return res.status(400).json({ success: false, message: 'Bot đang chạy!' });
    autoBotTask = cron.schedule('*/30 * * * *', () => autoRunCrawler());
    isBotRunning = true;
    return res.status(200).json({ success: true, message: 'Đã BẬT Auto-Pilot.' });
  } 
  if (action === 'stop') {
    if (!isBotRunning) return res.status(400).json({ success: false, message: 'Bot đã tắt!' });
    autoBotTask.stop();
    isBotRunning = false;
    return res.status(200).json({ success: true, message: 'Đã TẮT Auto-Pilot.' });
  }
};

const getBotStatus = (req, res) => res.status(200).json({ success: true, isRunning: isBotRunning });

module.exports = { 
  runCrawlerBot, autoRunCrawler, getPendingRooms, 
  approveRoom, toggleAutoBot, getBotStatus, deleteRoom
};